...
import { useApplicationVolunteer } from '../../hooks/useVolunteerApi';
...
export const AvailableApplication = ({
  ...
  const { mutateAsync, isPending } = useApplicationVolunteer();

  const handleApply = async () => {
    try {
      await mutateAsync(volunteerId);
      onApply(); // 부모 컴포넌트에 알림
  ...
  return (
    <Wrapper Theme={userTheme}>
      ...
      <Button
        Theme={userTheme}
        onClick={handleApply}
        disabled={status === 'APPLYING' || status === 'APPLIED' || isPending}
      >
        {isPending ? '신청 중...' : getButtonLabel()}
      </Button>
    </Wrapper>
  );
};
